    ...
    return (
        <div className="sim-controls card">
            <div className="card-body d-flex flex-row">
                {(state == "playing") ? (
                    <Button variant="" size="sm" title="Pause Simulation">
                        <i className="fas fa-pause text-warning"></i>
                    </Button>
                ) : (<>
                    <Button variant="" size="sm" title="Run Simulation" disabled={state == "done"} >
                        <i className="fas fa-play text-success"></i>
                    </Button>
                    <Button variant="" size="sm" title="Step Simulation" disabled={state == "done"}>
                        <i className="fas fa-step-forward text-success"></i>
                    </Button>
                </>)}
                {(state != "unstarted") ? (
                    <Button variant="" size="sm" title="Restart Simulation">
                ...
                <div className="flex-grow-1"> {/* Spacer, even if the slider is hidden */}
                    {(state == "playing") ? ( // 2**x steps per second
                        <input type="range" className="form-range" title="Speed" min={-2} max={4} value={speedTick}/>
                    ) : ""}
                </div>
